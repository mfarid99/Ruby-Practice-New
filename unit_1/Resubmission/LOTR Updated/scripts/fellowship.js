...
const makeMiddleEarth = () => {
  ...
  for (land of lands) {
    const $art = $('<article>').attr('id', land).append($('<h1>').text(land))
    $midEarth.append($art)
    }
  };
...
const makeBaddies = () => {
  ...
  const $baddies = $('<ul>')
  for (bad of baddies) {
    const $bad = $('<li>').addClass('baddy').text(bad)
    $baddies.append($bad)
  }
...
const makeBuddies = () => {
  ...
  for (bud of buddies) {
    const $bud = $('<li>').addClass('buddy').text(bud);
    $buddies.append($bud)
...
const beautifulStranger = () => {
      ...
      $('aside').children().eq(0).children().eq(3).text('Aragorn')
...
const forgeTheFellowShip = () => {
    ...
    $fellow.append($('<h1>').text('The Fellowship'))
...
const itsDangerousToGoAlone = () => {
  ...
  $('#Mordor').append($frodo).append($sam).append($('<div>')).attr('id', 'mount-doom')
...
const weWantsIt = () => {
      ...
      // $('#mount-doom').append($('<div>').attr('id', 'gollum'));
